...
- `B` (flake8-bugbear) ruff rules are deferred: 5 `pytest.raises(Exception)`
  ...
  before they can be tightened — not a mechanical fix.
- mypy is not adopted: `__init__.py`'s wrapper methods (`set_pi`,
  ...
  `__init__.py` first, not just an empty config.
...
- Stale FetchContent pin risk: this repo prefers a local `../libhmm`
  ...
  No periodic check currently exists for this.
...
which won't surface a stale pin.
...
- Triage the 5 `B017` blind-exception test assertions, then enable `B`.
